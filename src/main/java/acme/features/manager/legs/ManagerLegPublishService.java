...
public class ManagerLegPublishService extends AbstractGuiService<Manager, Leg> {
			...
			if (optionalLeg.isEmpty())
				...
				if (!leg.isDraftMode())
					...
					if (super.getRequest().hasData("departureAirport")) {
						departureId = super.getRequest().getData("arrivalAirport", int.class);
	...
	public void bind(final Leg leg) {
		...
		departure = this.repository.findAirportByAirportId(arrivalAirportId);
		departureAirportId = super.getRequest().getData("departureAirport", int.class);
		arrival = this.repository.findAirportByAirportId(departureAirportId);
